...
data "aws_ec2_instance_type_offerings" "my_ins_type" {               # check if t3micro is supported        
  ...
  filter {
    name   = "location"
    values = [each.key]                                               # use each key fromaws_ec2_instance_type_offerings output and iterate
  }
...
#output in may key and value eg us-east-1 = t3micro
output "output_v3_1" {
  value = {
...
#filter out az that t3micro is not supported
